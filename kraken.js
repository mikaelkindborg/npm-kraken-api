...
function KrakenClient(key, secret, options) {
  ...
  function api(method, params, callback) {
    ...
    if(methods.public.indexOf(method) !== -1) {
      ...
      throw new Error('Invalid Kraken API method: ' + method);
    }
  }
  ...
  function rawRequest(url, headers, params, callback) {
    ...
    var req = request.post(options, function(error, response, body) {
      if(typeof callback === 'function') {
        ...
        if(error) {
          return callback({ type: 'RequestError', error: error }, null);
        }
        ...
        try {
          ...
          return callback({ type: 'ResponseError', data: data }, null);
        }
        
        if(data && data.error && data.error.length > 0) { // && Array.isArray(data.error)) {
          callback({ type: 'KrakenError', error: data.error, data: data }, null);
        }
        else {
          callback(null, data);
        }
